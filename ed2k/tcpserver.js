...
const Packet = require('./packet.js').Packet;
const TcpCrypt = require('./tcpcrypt.js').TcpCrypt;
const conf = require('../enode.config.js').config;
const lowIdClients = require('./lowidclients.js').lowIdClients;
const op = require('./tcpoperations.js');
    ...
    server.on('error', function (err) {
        switch (err.code) {
            case 'EADDRNOTAVAIL':
                ...
                break
            default:
                log.error('Server error: ' + JSON.stringify(err))
        }
    })
