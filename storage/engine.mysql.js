const mysql = require('mysql'), // https://github.com/felixge/node-mysql
...
const sql = {
  ...
  esc: function (value) {
    return sql.connections[sql.current].escape(value);
  },
...
var files = {
    ...
    add: function (file, clientInfo) {
        ...
        sql.query('INSERT INTO files SET time_offer=NOW(), ? ON DUPLICATE KEY UPDATE time_offer=NOW()', {
            hash: file.hash,
            size: file.size
        }, function (err) {
            if (err) {
    ...
    addSource: function (file, clientInfo) {
        if (!file.type) {
            file.type = misc.getFileType(file.name);
        }
      const v = {
      ...
      if (file.codec) v.codec = file.codec;
        ...
        sql.query('INSERT INTO sources SET ?, time_offer = NOW() ON DUPLICATE KEY UPDATE ?, time_offer = NOW()', [v, v], function (err) {
            if (err) {
                return sql.defErr(err, 'files.addSource (insert)');
            }
            sql.current = clientInfo.id % sql.connectionsCount;
            sql.query('UPDATE files AS f ' +
                ...
                if (err) {
                    return sql.defErr(err, 'files.addSource (update)');
                }
            });
        });
    },
...
var searchExpr = function (token, type, value) {
    switch (type) {
        ...
        case 0x02:
            return " AND NOT ";
        case 0x030001:
            return "s.type=" + sql.esc(value);
        case 0x040001:
            return "s.ext=" + sql.esc(value);
        case 0xd50001:
            return "s.codec=" + sql.esc(value);
        case 0x02000101:
            return "f.size>" + sql.esc(value);
        case 0x02000102:
            return "f.size<" + sql.esc(value);
        case 0x15000101:
            return "f.sources>" + sql.esc(value);
        case 0xd4000101:
            return "s.bitrate>" + sql.esc(value);
        case 0xd3000101:
            return "s.duration>" + sql.esc(value);
        case 0x30000101:
            return "f.completed>" + sql.esc(value);
        default:
            log.warn('searchExpr: unknown type: 0x' + type.toString(16) + '; token: 0x' + token.toString(16) + '; value: ' + value);
    }
}
